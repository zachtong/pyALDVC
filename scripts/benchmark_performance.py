...
def bench(shape, winsize, step, admm=True):
    ...
    return {
        ...
        "admm": admm,
        "total_s": total,
        "init_s": res.timings.get("init_guess", 0.0),
        "precompute_s": res.timings.get("reference_precompute", 0.0),
        "local_s": res.timings.get("local_icgn", 0.0),
        "subpb1_s": res.timings.get("subpb1", 0.0),
        "subpb2_s": res.timings.get("subpb2", 0.0),
        "strain_s": res.timings.get("strain", 0.0),
        "nodes_per_s_local": n / max(res.timings.get("local_icgn", 1e-9), 1e-9),
        "mean_iter_local": n_local_iter,
        "admm_steps": fr.admm.n_steps if fr.admm else 0,
    }
...
def main(quick: bool) -> None:
    ...
    for shape, ws, st, admm in cases:
        ...
        rows.append(r)
        print(
            f"{r['shape']:>13} ws={ws:<3} step={st:<3} admm={admm!s:<5} nodes={r['nodes']:>6} total={r['total_s']:7.1f}s "
    ...
    with PdfPages(str(pdf_path)) as pdf:
        ...
        fig.text(
            0.04,
            0.95,
            f"pyALDVC {__version__} performance ({platform.node()}, {get_num_threads()} threads)",
            fontsize=13,
            weight="bold",
            va="top",
        )
        ...
        y = 0.9
        hdr = (
            f"{'shape':>13} {'ws':>3} {'step':>4} {'admm':>5} {'nodes':>6} {'total':>7} {'init':>6} {'pre':>6}"
            f" {'local':>6} {'sub1':>6} {'sub2':>5} {'strain':>6} {'local nodes/s':>13}"
        )
        fig.text(0.04, y, hdr, fontsize=8, family="monospace", va="top")
        y -= 0.025
        for r in rows:
            fig.text(
                0.04,
                y,
                f"{r['shape']:>13} {r['winsize']:>3} {r['step']:>4} {str(r['admm']):>5} {r['nodes']:>6} {r['total_s']:7.1f} "
                f"{r['init_s']:6.1f} {r['precompute_s']:6.1f} {r['local_s']:6.1f} {r['subpb1_s']:6.1f} {r['subpb2_s']:5.2f} "
                f"{r['strain_s']:6.2f} {r['nodes_per_s_local']:13.0f}",
                fontsize=8,
                family="monospace",
                va="top",
            )
        ...
        fig.text(
            0.04,
            y - 0.02,
            "Times in seconds (JIT already warm). 'local' = 12-DOF IC-GN over all nodes; 'sub1' = all 3-DOF ADMM passes.",
            fontsize=8,
            va="top",
        )
        pdf.savefig(fig)
        ...
        axes[0].bar(
            [f"{r['shape']}\nws{r['winsize']} s{r['step']}" for r in sub], [r["local_s"] for r in sub], label="local IC-GN"
        )
        axes[0].bar(
            [f"{r['shape']}\nws{r['winsize']} s{r['step']}" for r in sub],
            [r["subpb1_s"] for r in sub],
            bottom=[r["local_s"] for r in sub],
            label="ADMM 3-DOF passes",
        )
        axes[0].bar(
            [f"{r['shape']}\nws{r['winsize']} s{r['step']}" for r in sub],
            [r["init_s"] + r["precompute_s"] for r in sub],
            bottom=[r["local_s"] + r["subpb1_s"] for r in sub],
            label="init guess + precompute",
        )
        ...
        axes[1].set_xlabel("nodes x subset voxels")
        axes[1].set_ylabel("seconds")
        axes[1].grid(alpha=0.3, which="both")
        axes[1].legend()
        axes[1].set_title("Local pass scales with total subset voxels")
        fig.tight_layout()
        pdf.savefig(fig)
        plt.close(fig)
