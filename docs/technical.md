...
| 1024 x 1024 x 306 confocal (MATLAB example) | 32 / 8 | 79 200 | **CPU 5.0 min, RTX 5090: 34 s** with the current defaults (0.3.x with the noise-corrected steps on: CPU 3.6 min, 3.2 min with `init_coarse_factor=2`, GPU 23 s; 12.5 min before the 0.3.2 work) | CPU 1.2 min + 1.5 min for three 3-DOF passes, initial guess 0.9 min (0.3 min coarse); GPU 2.6 s + 6.2 s, initial guess 9.9 s |
...
`scripts/compare_matlab.py` reruns the confocal example shipped with the
...
The run takes 5.0 min on a 24-core workstation and 34 s on an RTX 5090 with the current defaults (measured 2026-09-23 on an idle machine; 3.6 min and 23 s in 0.3.x with the noise-corrected steps, then the default). See `docs/design.md`,
section 10.
