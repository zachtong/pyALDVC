...
| 0.2 | "real-scan ready": deformed-frame masks in the kernel (done), per-node displacement uncertainty from the stored Hessian factors (done), per-frame checkpoints and resume for long sequences, large-volume mode (on-the-fly gradients, memory budget), real-data tutorial notebook, PyPI release |
