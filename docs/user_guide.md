...
| `subset_split` | False | a subset that touches a hole, the region edge or a crack keeps only the part connected to its centre (6-connected, decided at full resolution), and the node grid is cut at the same boundaries, so the global step, the inpainting, the median test and the strain fit stay on one side of it; `min_valid_ratio` then applies to the kept part and the kept share of every subset is exported as `split_fraction`. The initial guess still uses the whole window. CPU kernels: with the CUDA backend the local steps run on the CPU while this is on |
