...
the analyses pyALDVC was built for, and the results agree with the MATLAB ALDVC. The project now has a website,
https://zachtong.github.io/pyALDVC/, with case studies and the principles behind each tool.

### Added
- **The project website** (`site/`, published by `.github/workflows/pages.yml`): cases on DVC Challenge 2.0
  data (foam compression by micro-CT, hydrogel indentation by confocal microscopy, a synthetic rigid-body
  rotation), the features, how the method, tracking, texture analysis, error analysis and rigid-body removal
  work, the application, accuracy and speed, install and citation. Its media and figures are generated by
  `scripts/make_site_media.py`, `make_site_figures.py` and `make_site_screens.py`.
- **A redesigned README** in the website's style, with a link to the website at the top: result animations, the
  key numbers, a case gallery, the features, how it works, install and citation; the full guide stays in
  `docs/user_guide.md`. Its buttons and numbers strip are drawn with the site's style sheet by
  `scripts/make_readme_images.py`.

### Changed
- **The noise-corrected Gauss-Newton steps are an advanced option, off by default.** They halve the
  subset iterations on noisy scans with the same answer, but assume that both scans carry comparable
  noise; with a clearly cleaner reference -- an averaged scan, or synthetic noise added to a copy of
  the reference as in the DVC Challenge noise-floor test -- they over-correct and many subsets stopped
  without converging (static test with 2 % noise in the deformed scan only: 52 % of the nodes converged
  with AL-DVC and 10 % with Local DVC alone, instead of 100 %). The MATLAB ALDVC has no such step.
  *Advanced > Noise-corrected steps* turns them on and says when they apply. Sessions saved by an
  earlier version open with them off and say so; to reproduce their results, or to resume checkpoints
  written by an earlier version (the application would otherwise start such a run over), tick the option
  first. On the confocal example (1024 x 1024 x 306 voxels, 79,200 nodes) the default run takes 34 s on
  an RTX 5090 and 5.0 min on a 24-core CPU (about 32 s and 4.0 min with the option on), and slightly
  more nodes converge with it off.
